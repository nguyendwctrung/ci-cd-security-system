...
from security_system.config.constants import REPORTS_DIR
...
class TestSecurityPipelineE2E:
    ...
    def test_pipeline_generates_summary_report(
        ...
        with patch(
            ...
            assert summary_dict["total_findings"] == 1
    ...
    @patch("security_system.monitor.pipeline.GitService")
    @patch("security_system.monitor.pipeline.LLMAnalyzer")
    def test_pipeline_full_run_pass_decision(
        self, mock_llm_analyzer_class, mock_git_service_class, mock_reports_dir
    ):
